...
const QualityGateView: React.FC<QualityGateViewProps> = ({ channels, users, masterOptions, onOpenTask, currentUser, tasks }) => {
    ...
    return (
        <AppBackground theme="inspector" pattern="grid" className="p-4 sm:p-6 md:p-8 min-h-screen">
            <div className="space-y-6 animate-in fade-in duration-500 pb-20 relative z-10">
                <div className="max-w-[1600px] mx-auto space-y-8">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                        <div className="flex items-center gap-3 w-full justify-between md:justify-start">
                            <div className="flex items-center gap-3">
                                <div className="p-2.5 sm:p-4 bg-indigo-500/10 text-indigo-400 rounded-2xl sm:rounded-3xl shadow-2xl border border-indigo-500/20 backdrop-blur-md shrink-0">
                                    <Layers className="w-6 h-6 sm:w-10 sm:h-10" />
                                </div>
                                <div className="min-w-0">
                                    <h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-white flex flex-wrap items-center tracking-tight italic uppercase gap-2 leading-none">
                                        Inspector's Desk <span className="text-indigo-500 not-italic text-xl sm:text-2xl">🔍</span>
                                    </h1>
                                    <p className="text-indigo-400/60 mt-1 font-black uppercase tracking-[0.2em] sm:tracking-[0.3em] text-[9px] sm:text-xs truncate">
                    ...
                    <div className="flex flex-col gap-4 top-4 z-30">
                        {/* Top Row: Search */}
                        <div className="bg-slate-900/80 backdrop-blur-2xl p-2.5 sm:p-3 rounded-2xl sm:rounded-3xl border border-white/10 shadow-2xl ring-1 ring-white/5">
                            <div className="relative group">
                                <Search className="absolute left-4 sm:left-5 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 h-5 text-indigo-500 transition-colors group-focus-within:text-indigo-400" />
                                <input 
                                    type="text" 
                                    placeholder="SCANNING FOR TASKS (TITLE, TAGS)..." 
                                    className="w-full pl-11 sm:pl-14 pr-6 py-3.5 sm:py-4 rounded-xl sm:rounded-2xl bg-black/60 border border-white/10 focus:border-indigo-500/50 focus:ring-8 focus:ring-indigo-500/10 outline-none transition-all text-xs sm:text-sm font-black text-white placeholder:text-indigo-900/40 uppercase tracking-[0.15em] sm:tracking-[0.2em]"
                                    value={searchTerm}
                                    onChange={(e) => setSearchTerm(e.target.value)}
                                />
                                <div className="absolute right-4 sm:right-5 top-1/2 -translate-y-1/2 flex items-center gap-1.5 sm:gap-2">
                                    <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-indigo-500 animate-pulse shadow-[0_0_10px_rgba(99,102,241,0.8)]"></div>
                                    <span className="text-[8px] sm:text-[10px] font-black text-indigo-500/40 tracking-widest uppercase hidden xs:inline">Active Scan</span>
                                </div>
                            </div>
                        </div>

                        {/* Bottom Row: Filters & Team */}
                        <div className="bg-slate-900/60 backdrop-blur-2xl p-4 rounded-2xl sm:rounded-[2.5rem] border border-white/5 shadow-2xl flex flex-col xl:flex-row items-stretch xl:items-center gap-4 sm:gap-6 ring-1 ring-white/5">
                            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 shrink-0 w-full xl:w-auto">
                                ...
                                <button
                                    ...
                                    disabled={isChecking}
                                    className={`flex items-center justify-center gap-2 px-4 sm:px-5 py-3 sm:py-3.5 rounded-xl sm:rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all border shrink-0 ${
                                        isChecking 
                                ...
                                >
                                    <RefreshCw className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${isChecking ? 'animate-spin' : ''}`} />
                                    {isChecking ? 'Checking...' : 'Check SLA'}
                                </button>

                                <div className="flex bg-black/40 p-1 sm:p-1.5 rounded-xl sm:rounded-2xl border border-white/5 shrink-0 shadow-inner w-full sm:w-auto overflow-x-auto scrollbar-none">
                                    <button onClick={() => setFilterDateType('ALL_PENDING')} className={`flex-1 sm:flex-none px-3 sm:px-6 py-2.5 sm:py-3 rounded-lg sm:rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${filterDateType === 'ALL_PENDING' ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-900/50' : 'text-indigo-400/40 hover:text-indigo-400 hover:bg-white/5'}`}>
                                        Pending ({totalActiveTasks})
                                    </button>
                                    <button onClick={() => setFilterDateType('TODAY')} className={`flex-1 sm:flex-none px-3 sm:px-6 py-2.5 sm:py-3 rounded-lg sm:rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${filterDateType === 'TODAY' ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-900/50' : 'text-indigo-400/40 hover:text-indigo-400 hover:bg-white/5'}`}>
                                        Today
                                    </button>
                                    <button onClick={() => setFilterDateType('OVERDUE')} className={`flex-1 sm:flex-none px-3 sm:px-6 py-2.5 sm:py-3 rounded-lg sm:rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${filterDateType === 'OVERDUE' ? 'bg-rose-600 text-white shadow-xl shadow-rose-900/50' : 'text-indigo-400/40 hover:text-rose-400 hover:bg-white/5'}`}>
                        ...
                        <div className="space-y-10 sm:space-y-12">
                            {groups.critical.length > 0 && (
                                <section className="animate-in slide-in-from-left-4 duration-500">
                                    <button onClick={() => toggleGroup('CRITICAL')} className="flex items-center justify-between w-full mb-4 sm:mb-6 group text-left">
                                        <h3 className="text-sm sm:text-lg md:text-xl font-black text-rose-500 flex items-center bg-rose-500/10 px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl border border-rose-500/20 shadow-xl backdrop-blur-md italic uppercase tracking-tight min-w-0">
                                            <AlertTriangle className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 mr-2 sm:mr-3 shrink-0 animate-pulse" /> 
                                            <span className="truncate">Critical / Overdue ({groups.critical.length})</span>
                                        </h3>
                                        <div className="h-px bg-rose-500/10 flex-1 mx-2 sm:mx-6 group-hover:bg-rose-500/30 transition-colors hidden xs:block"></div>
                                        {collapsedGroups['CRITICAL'] ? <ChevronRight className="text-rose-500/40 shrink-0" /> : <ChevronDown className="text-rose-500/40 shrink-0" />}
                                    </button>
                                    <AnimatePresence>
                            ...
                            {groups.revise.length > 0 && (
                                <section className="animate-in slide-in-from-left-4 duration-500 delay-100">
                                    <button onClick={() => toggleGroup('REVISE')} className="flex items-center justify-between w-full mb-4 sm:mb-6 group text-left">
                                        <h3 className="text-sm sm:text-lg md:text-xl font-black text-amber-500 flex items-center bg-amber-500/10 px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl border border-amber-500/20 shadow-xl backdrop-blur-md italic uppercase tracking-tight min-w-0">
                                            <LayoutList className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 mr-2 sm:mr-3 shrink-0" /> 
                                            <span className="truncate">Revision Queue ({groups.revise.length})</span>
                                        </h3>
                                        <div className="h-px bg-amber-500/10 flex-1 mx-2 sm:mx-6 group-hover:bg-amber-500/30 transition-colors hidden xs:block"></div>
                                        {collapsedGroups['REVISE'] ? <ChevronRight className="text-amber-500/40 shrink-0" /> : <ChevronDown className="text-amber-500/40 shrink-0" />}
                                    </button>
                                    <AnimatePresence>
                            ...
                            {groups.today.length > 0 && (
                                <section className="animate-in slide-in-from-left-4 duration-500 delay-150">
                                    <button onClick={() => toggleGroup('TODAY')} className="flex items-center justify-between w-full mb-4 sm:mb-6 group text-left">
                                        <h3 className="text-sm sm:text-lg md:text-xl font-black text-indigo-400 flex items-center bg-indigo-500/10 px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl border border-indigo-500/20 shadow-xl backdrop-blur-md italic uppercase tracking-tight min-w-0">
                                            <Clock className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 mr-2 sm:mr-3 shrink-0" /> 
                                            <span className="truncate">Today's Inspection ({groups.today.length})</span>
                                        </h3>
                                        <div className="h-px bg-indigo-500/10 flex-1 mx-2 sm:mx-6 group-hover:bg-indigo-500/30 transition-colors hidden xs:block"></div>
                                        {collapsedGroups['TODAY'] ? <ChevronRight className="text-indigo-400/40 shrink-0" /> : <ChevronDown className="text-indigo-400/40 shrink-0" />}
                                    </button>
                                    <AnimatePresence>
                            ...
                            {groups.upcoming.length > 0 && (
                                <section className="animate-in slide-in-from-left-4 duration-500 delay-200">
                                    <button onClick={() => toggleGroup('UPCOMING')} className="flex items-center justify-between w-full mb-4 sm:mb-6 group opacity-50 hover:opacity-100 transition-opacity text-left">
                                        <h3 className="text-xs sm:text-md font-black text-indigo-400/60 flex items-center bg-white/5 px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl border border-white/5 italic uppercase tracking-widest min-w-0">
                                            <Calendar className="w-4 h-4 sm:w-5 sm:h-5 mr-2 sm:mr-3 shrink-0" /> 
                                            <span className="truncate">Upcoming Queue ({groups.upcoming.length})</span>
                                        </h3>
                                        <div className="h-px bg-white/5 flex-1 mx-2 sm:mx-6 border-dashed border-b border-white/10 hidden xs:block"></div>
                                        {collapsedGroups['UPCOMING'] ? <ChevronRight className="text-indigo-400/20 shrink-0" /> : <ChevronDown className="text-indigo-400/20 shrink-0" />}
                                    </button>
                                    <AnimatePresence>
